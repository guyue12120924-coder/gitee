...
// ========== 新增：主题切换 & 加载动画 & 图片预览 ==========
function initTheme() {
  const stored = localStorage.getItem('moark_theme');
...
function toggleTheme() {
  ...
  localStorage.setItem('moark_theme', newTheme);
...
async function runHunyuanVideo() {
  ...
  try {
    ...
    addOutputItem({ title:"HunyuanVideo 错误 / Error", meta:String(e) });
  } finally {
    showLoading(false);
  }
}
...
async function runZImage() {
  ...
  try {
    ...
    addOutputItem({ title:"z-image 错误 / Error", meta:String(e) });
  } finally {
    showLoading(false);
  }
}
...
async function runEdit() {
  ...
  try {
    ...
    addOutputItem({ title:"Edit-2511 错误 / Error", meta:String(e) });
  } finally {
    showLoading(false);
  }
}
...
async function runWan() {
  ...
  try {
    ...
    addOutputItem({ title:"Wan2.2 错误 / Error", meta:String(e) });
  } finally {
    showLoading(false);
  }
}
...
function initUi() {
  ...
  setupImagePreview('editImg1', 'editImg1Preview');
  setupImagePreview('editImg2', 'editImg2Preview');
  setupImagePreview('wanImg', 'wanImgPreview');
}
