(() => {
  ...
  function enhanceResultItem(item) {
    ...
    edit.addEventListener("click", (event) => {
      ...
      sendImageTo("edit", image.currentSrc || image.src);
    });
    ...
    video.addEventListener("click", (event) => {
      event.stopPropagation();
      sendImageTo("i2v", image.currentSrc || image.src);
    });
  ...
  function syncStepper(input) {
    const value = input.parentElement?.querySelector(":scope > .studio-stepper .studio-stepper-value");
  ...
  function simplifyParameterPanel(panel) {
    ...
    for (const section of panel.querySelectorAll(":scope > .mp-section")) {
      ...
      if (/常用参数/.test(text) || section.classList.contains("studio-primary-params")) {
  ...
  function simplifyInspectorChrome() {
    for (const summary of document.querySelectorAll(".studio-model-developer-tools > summary")) {
      if (summary.textContent !== "开发者设置") summary.textContent = "开发者设置";
    }
  ...
  function setupParameterPolish() {
    ...
    // Deliberately event-driven. Do NOT observe workspaceInspectorHost subtree:
    // textContent updates inside the inspector can otherwise trigger an endless MutationObserver loop.
    for (const id of MODEL_SELECT_IDS) {
      $(id)?.addEventListener("change", () => setTimeout(schedule, 0));
    }
    document.addEventListener("change", (event) => {
      const id = event.target?.id || "";
      if (/^(zRes|zN|mmI2VRatio|mmI2VResolution|hyAspect|mmT2VResolution)$/.test(id)) schedule();
    });
    window.GiteeStudioPolishParameters = schedule;
  }
  ...
  function injectRefinementStyles() {
    ...
    style.textContent = `
      @media (min-width:1181px){.workspace-shell{grid-template-columns:58px minmax(0,1fr) minmax(300px,318px)!important;gap:8px!important}}
      @media (min-width:901px) and (max-width:1180px){.workspace-shell{grid-template-columns:56px minmax(0,1fr) 300px!important;gap:8px!important}}
      .workspace-inspector-state{width:8px!important;min-width:8px!important;height:8px!important;min-height:8px!important;padding:0!important;border-radius:50%!important;font-size:0!important;background:#94a3b8!important;color:transparent!important}
      .workspace-inspector-state.is-pass{background:#22c55e!important}.workspace-inspector-state.is-experimental{background:#f59e0b!important}.workspace-inspector-state.is-fail{background:#ef4444!important}.workspace-inspector-state.is-custom{background:var(--accent)!important}
      .workspace-inspector .mp-head{display:none!important}.workspace-inspector .mp-panel{padding:0!important;margin:0!important;border-bottom:0!important}
      .studio-primary-params{margin:0!important;padding:0!important;border:0!important}.studio-primary-params>summary{display:none!important}.studio-primary-params>.mp-grid{display:grid!important;grid-template-columns:1fr!important;gap:14px!important;padding:2px 0 0!important}.studio-primary-params .mp-field-help{display:none!important}
      .studio-advanced-params{margin:12px 0 0!important;padding:10px 0 0!important;border-top:1px solid var(--border-light)!important}.studio-advanced-params>summary,.studio-model-developer-tools>summary{padding:3px 0!important;color:var(--muted)!important;font-size:11px!important;font-weight:650!important}
      .studio-native-parameter-control{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:0!important;opacity:0!important;pointer-events:none!important;overflow:hidden!important}
      .studio-choice-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.studio-choice-button{min-height:46px;padding:6px 5px;border:1px solid var(--border-light);border-radius:10px;background:var(--studio-soft);color:var(--muted);cursor:pointer;text-align:center;transition:.14s ease}.studio-choice-button:hover{border-color:rgba(99,102,241,.28);color:var(--text)}.studio-choice-button.is-active{border-color:rgba(99,102,241,.46);background:rgba(99,102,241,.10);color:var(--accent)}.studio-choice-button strong{display:block;font-size:12px;line-height:1.2}.studio-choice-button span{display:block;margin-top:3px;font-size:9px;color:var(--muted)}
      .studio-stepper{display:grid;grid-template-columns:38px minmax(50px,1fr) 38px;align-items:center;width:min(170px,100%);height:40px;border:1px solid var(--border-light);border-radius:10px;overflow:hidden;background:var(--input-bg)}.studio-stepper-button{height:100%;border:0;background:transparent;color:var(--muted);font-size:19px;cursor:pointer}.studio-stepper-button:hover{background:var(--btn-bg);color:var(--text)}.studio-stepper-value{text-align:center;color:var(--text);font-size:13px;font-weight:750}
      .workspace-composer{padding:6px 8px!important;border-radius:14px!important}.workspace-composer-input{min-height:52px!important;max-height:108px!important;padding:9px 11px!important}.workspace-composer-actions{height:46px!important}.workspace-generate-button{width:104px!important;min-width:104px!important;height:46px!important}
      @media(max-width:900px){.workspace-shell{grid-template-columns:1fr!important;grid-template-rows:auto minmax(520px,auto) auto!important;gap:8px!important}.workspace-composer-input{min-height:50px!important;max-height:96px!important}.workspace-generate-button{width:92px!important;min-width:92px!important}}
      @media(max-width:560px){.studio-choice-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    `;
